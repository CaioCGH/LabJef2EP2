...
class t_apply_questionary(WEDWorkerTemplate):
    
    #trname and dbs variables are static in order to conform with the definition of wed_trans()    
    trname = 't_apply_questionary'
    dbs = 'user=organ_donation dbname=organ_donation application_name=t_apply_questionary'
    wakeup_interval = 5
    ...
    def wed_trans(self, wed_state, wid):
        print("\033[33m> Appling questionary (Id = %d)\033[0m" % wid)
        print ("\033[33m> WED-state: %s\033[0m" % wed_state)
        
        #business logic goes here 
        
        return "questionary_status = 'Applied'"
        
w = t_apply_questionary()
